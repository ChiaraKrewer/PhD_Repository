...
OP_DP = y(20) ;
O2_A = y(21) ;
A = y(22) ; %%inorganic carbon
Aiso = y(23) ; %inorganic carbon isotopes
...
%%%%% mixing ratio of oxygen (not proportional to O reservoir) - from SCION
%%%%% for O2 calculations in %
...
Pwflux= 0e10 ; %add P weathering influx for test runs 
Pflux= 1.9e10 ; %%LIP-related P input, set to 1.9e10 for all CO2 ranges
...
D = interp1([-4.2e9 -3e9 -2e9 -10e7 0],[12 5 2 2 1],t) * sensparams.D; %increase D for 100Ma to 2
...
% including CO2 input
dy(22) = Foxidw  + Focdeg + Fcarbw + Fccdeg - Fmocb - Fmccb - Fsfw + Frgf - Flocb + CO2_input;
dy(23) = ((Foxidw*dG) + (Focdeg*dG) + (Fcarbw*dC) + (Fccdeg*dC) + (Frgf*dM) - (Fmocb*delta_mocb) - (Fmccb*delta_mccb) - (Fsfw*delta_mccb) - (Flocb*(delta_locb)) +(CO2_input*d13c_CO2_input) )/A ; %inorganic carbon isotopes
...
%% Phosphorus Differentials
    ...
    %SRP Proximal
    dy(13) = River_SRP - P_PP_P + OP_P_Min - P_FeP_P - P_AuthP_P - SRP_P_D + add_Pw_input; 
    ...
    %SRP Deep Ocean
    dy(19) = SRP_S_DP + OP_DP_Min - P_FeP_DP - P_AuthP_DP - SRP_DP_S - SRP_DP_D - Fe_SCAV_SURF  + add_P_input  ;
...
workingstate.Fw(stepnumber,1) = Fw ;
