function dy = MBOX_C_equations_github(t,y)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%% tested forcings
pars.FeIIhydro = 1.35e10;
pars.k_ccdeg = 8e12;
pars.k_ocdeg = 1.25e12; %% why is this constant not included in Ming's initial script? ccdeg is included based on Dal Corso et al., 
% phosw = pars.k_phosw;
...
t1 = -94.495;
t2 = -94.065;
rate = 43.32e12;%% between 7.22e12 and 14.44e12 for 430ka which gives a total of 3.25 to 6.5e18 CO2 (within lit range)
slope = 800;
CO2_input = rate * ( sigmf(t_geol,[slope t1]) - sigmf(t_geol,[slope t2]) );

% %%% additiona Nutrient flux
Feflux= 0e12 ; %%LIP-related Fe flux, set to 1.3e12 for all CO2 ranges
add_Fe_input = Feflux * ( sigmf(t_geol,[slope t1]) - sigmf(t_geol,[slope t2]) ); %can be added to deep or distal, if to proximal model response different
Pflux= 0e10 ; %%LIP-related P flux, does not change anything in the runs
add_P_input = Pflux * ( sigmf(t_geol,[slope t1]) - sigmf(t_geol,[slope t2]) );


Fewflux= 0e11 ; %%additional Fe weathering flux for test runs, set to 5e11for all CO2 ranges
add_Few_input = interp1([-150 -94.55 -94.54 -94.40 -94.39 -93 -50]',[0 0 Fewflux Fewflux 0 0 0]',t_geol); %can be added to deep or distal, if to proximal model response different
...
Climate_Sensitivity = 5 ;%slightly increased climate sensitivity, was 3
GAST = 288 + Climate_Sensitivity * ( log( Atmospheric_CO2_ppm/280 ) / log(2) )  - 7.4*(t_geol/-570)  ;
T_s = 298 + (GAST - 288)*0.66  ;  
T_h = max( 275.5 + (GAST - 288)*1.5, 271 ) ;
T_d = max( 275.5 + (GAST - 288)*1.5, 271 ) ;
T_cont = GAST ;
T_p = T_s ;
T_di = T_s ;
...
DEGASS = 2.5; %slightly increased to match Cretaceous background CO2 and temperature, after Marcilly et al
Bforcing = 1 ;
...
%%%% FeIII scavenging
%%% Scavenging rate unable to change as model will break
FeIIIscavenging_p  = 1*(max(3.18e+18*(FeIII_conc_p - 0.58e-6),0));
FeIIIscavenging_di = 1*(max(1.54e+18*(FeIII_conc_di - 0.58e-6),0));
FeIIIscavenging_s  = 1*(max(3.63e+18*(FeIII_conc_s - 0.58e-6),0));
FeIIIscavenging_h  = 1*(max(3.63e+18*(FeIII_conc_h - 0.58e-6),0));
FeIIIscavenging_d  = 1*(max(3.63e+18*(FeIII_conc_d - 0.58e-6),0));
...
% Organic C preservation with Fe oxides: 1 Fe bounds 4 C after Lalonde et
% al., 2012, but this is variable, see paper.
...
C_Poxic=250;
C_Panoxic=1000;
DOA_p= max(1.00 - 0.9975*O2_p/pars.O2_p_0 , 0 ) ;
...
DOA_di= max( 1.00 - 0.9975*O2_di/pars.O2_di_0 , 0 ) ;
...
DOA_d= max( 1.00 - 0.9975*O2_d/pars.O2_d_0 , 0 ) ;
...
%%%% DP_d including LIP input
dy(27) = phosbenthic_d + Tran_DP_h_d - Tran_DP_d_di - Tran_DP_d_s - Mix_DP_d_s - Mix_DP_d_h  + remin_d/pars.Red_C_P ;

% %%% DP concentration excluding benthic P flux
% %%%% DP_p
% dy(23) = phosw - Tran_DP_p_s + Tran_DP_di_p + Mix_DP_di_p  - pripr_p/pars.Red_C_P + remin_p/pars.Red_C_P;
% 
% %%%% DP_di
% dy(24) =  Tran_DP_d_di - Tran_DP_di_p - Mix_DP_di_p  + remin_di/pars.Red_C_P ;
% 
% %%%% DP_s
% dy(25) = Tran_DP_p_s - Tran_DP_s_h + Tran_DP_d_s + Mix_DP_d_s - pripr_s/pars.Red_C_P + remin_s/pars.Red_C_P ;
% 
...
% 
% %%%% DP_d including LIP input
% dy(27) = Tran_DP_h_d - Tran_DP_d_di - Tran_DP_d_s - Mix_DP_d_s - Mix_DP_d_h  + remin_d/pars.Red_C_P ;
...
mrO2= (O2_a/3.7e19) / ((O2_a/3.7e19) +3.762);
...
%%%% FeIII_p
dy(39) = add_Few_input + FeIIIw + Tran_FeIII_di_p - Tran_FeIII_p_s + Mix_FeIII_di_p - pripr_p/pars.Red_C_Fe + remin_p/pars.Red_C_Fe - 1*FeIIIscavenging_p - 4*ironR_p + ironO_p - 8*SironR_p;

%%%% FeIII_di
dy(40) =  Tran_FeIII_d_di - Tran_FeIII_di_p - Mix_FeIII_di_p + remin_di/pars.Red_C_Fe - 1*FeIIIscavenging_di - 4*ironR_di + ironO_di - 8*SironR_di;

%%%% FeIII_s
dy(41) = pars.FeIIIa_s + Tran_FeIII_p_s - Tran_FeIII_s_h + Tran_FeIII_d_s + Mix_FeIII_d_s - pripr_s/pars.Red_C_Fe + remin_s/pars.Red_C_Fe - 1*FeIIIscavenging_s - 4*ironR_s  + ironO_s - 8*SironR_s;

%%%% FeIII_h
dy(42) = pars.FeIIIa_h + Tran_FeIII_s_h - Tran_FeIII_h_d + Mix_FeIII_d_h - pripr_h/pars.Red_C_Fe + remin_h/pars.Red_C_Fe - 1*FeIIIscavenging_h - 4*ironR_h  + ironO_h - 8*SironR_h;

%%%% FeIII_d
dy(43) = - Tran_FeIII_d_di - Tran_FeIII_d_s + Tran_FeIII_h_d - Mix_FeIII_d_s - Mix_FeIII_d_h + remin_d/pars.Red_C_Fe - 1*FeIIIscavenging_d - 4*ironR_d  + ironO_d - 8*SironR_d;
...
% %%%% FeII_d not including any additional LIP Fe input
...
%%%% FeII_d including LIP input
dy(53) = FeIIbenthic_d + pars.FeIIhydro + Tran_FeII_h_d - Tran_FeII_d_di - Tran_FeII_d_s - Mix_FeII_d_s - Mix_FeII_d_h + 4*ironR_d - pyF_d - ironO_d + 8*SironR_d - SideP_d + add_Fe_input ;
...
% workingstate.LIP_P_input_d(stepnumber,1) = LIP_P_input_d ;
workingstate.add_Fe_input(stepnumber,1) = add_Fe_input ;
workingstate.add_Few_input(stepnumber,1) = add_Few_input ;
workingstate.add_P_input(stepnumber,1) = add_P_input;
...
workingstate.FeIIbenthic_di(stepnumber,1) = FeIIbenthic_di ;
workingstate.FeIIbenthic_d(stepnumber,1) = FeIIbenthic_d ;
workingstate.FeIIIw(stepnumber,1) = FeIIIw;
% workingstate.sFeIII_p(stepnumber,1) = sFeIII_p;
% workingstate.sFeIII_di(stepnumber,1) = sFeIII_di;
% workingstate.sFeIII_d(stepnumber,1) = sFeIII_d;
...
%workingstate.pyriteb_p(stepnumber,1) = pyriteb_p; %%% not caluclated
%workingstate.pyriteb_di(stepnumber,1) = pyriteb_di; %%% not caluclated
...
workingstate.phosbenthic_p(stepnumber,1) = phosbenthic_p;
workingstate.phosbenthic_di(stepnumber,1) = phosbenthic_di;
workingstate.phosbenthic_d(stepnumber,1) = phosbenthic_d;
...
workingstate.DOA_p(stepnumber,1) = DOA_p;
workingstate.DOA_di(stepnumber,1) = DOA_di;
workingstate.DOA_d(stepnumber,1) = DOA_d;

workingstate.CPratio_p(stepnumber,1) = CPratio_p;
workingstate.CPratio_di(stepnumber,1) = CPratio_di;
workingstate.CPratio_d(stepnumber,1) = CPratio_d;

workingstate.POPburial_p(stepnumber,1) = POPburial_p;
workingstate.POPburial_di(stepnumber,1) = POPburial_di;
workingstate.POPburial_d(stepnumber,1) = POPburial_d;

workingstate.PFe_p(stepnumber,1) = PFe_p;
workingstate.PFe_di(stepnumber,1) = PFe_di;
workingstate.PFe_d(stepnumber,1) = PFe_d;

workingstate.Pauth_p(stepnumber,1) = Pauth_p;
workingstate.Pauth_di(stepnumber,1) = Pauth_di;
workingstate.Pauth_d(stepnumber,1) = Pauth_d;

workingstate.total_mocb(stepnumber,1) = total_mocb;
workingstate.total_CO2(stepnumber,1) = total_CO2;
% workingstate.add_FeII_d(stepnumber,1) = add_FeIIbenthic_d ; 
% workingstate.add_FeII_p(stepnumber,1) = add_FeIIbenthic_p ; 
% workingstate.add_FeII_di(stepnumber,1) = add_FeIIbenthic_di ; 
% %workingstate.add_benthicFe(stepnumber,1) = add_benthicFe;
