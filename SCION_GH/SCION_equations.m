...
%%% Experiment OAE2
d13c_CO2_input = -5;
flux = 7.22e12 ; % CO2 flux between 7.22e12 and 14.44e12 for 430ka which gives a total of 3.25 to 6.5e18 CO2 (within literature range)
CO2_input = interp1qr([-300 -94.49 -94.48 -94.07 -94.06 -93 0]',[0 0 flux flux 0 0 0]',t_geol) ; %LIP duration 450ka results in CIE duration for ca ~690ka (till end of recovery)


%%% additional Nutrient flux
Pwflux= 0e10 ; %additional P weathering influx, for test runs
Pflux= 0e10 ; %% LIP-related P influxe, set to 5.5e10 for all CO2 ranges
add_P_input = interp1([-300 -94.49 -94.48 -94.07 -94.06 -93 0]',[0 0 Pflux Pflux 0 0 0]',t_geol); %can be added to deep or distal with same results, if to proximal model response different
add_Pw_input = interp1([-300 -94.55 -94.54 -94.40 -94.39 -93 0]',[0 0 Pwflux Pwflux 0 0 0]',t_geol); % additional P weathering influx
...
%%%% hi lat temperature (currently set to -60)
SAT_hilat = mean(mean( Tair_past(34,:)))*contribution_past  +  mean(mean( Tair_future(34,:) ))*contribution_future  ;
...
%%% Phosphate including additional P fluxes
dy(1) = psea - mopb - capb - fepb + add_P_input + add_Pw_input ;

%%% Oxygen
dy(2) = locb + mocb - oxidw  - ocdeg  + 2*(mpsb - pyrw  - pyrdeg) - reductant_input ;

%%% Carbon dioxide
dy(3) = -locb - mocb + oxidw + ocdeg + ccdeg + carbw - mccb - sfw  + reductant_input + CO2_input;
...
%%% delta_A * A
dy(16) = -locb*(  delta_locb ) -mocb*( delta_mocb ) + oxidw*delta_G + ocdeg*delta_G + ccdeg*delta_C + carbw*delta_C - mccb*delta_mccb - sfw*delta_mccb + reductant_input*-5 + CO2_input*d13c_CO2_input ;
...
if sensanal == 0
    ...
    workingstate.add_Pw_input(stepnumber,1) = add_Pw_input;
    ...
    workingstate.SAT_hilat(stepnumber,1) = SAT_hilat ;
